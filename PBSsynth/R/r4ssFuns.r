...
## Based on 'r4ss::SSplotComps'
## 'boxpars' created in call to 'zzz.local.r'
## -----------------------------------------------
plotSS.stdres <- function(replist, kind="AGE", fleets="all",
   ...
   type="Multinomial", useOSA=TRUE, addPearOSA=FALSE, addRawOSA=FALSE, usePearson=FALSE,
   rm0=FALSE, plot=TRUE, print=FALSE, ptypes="png", pngres=400, PIN=c(10,8.5),
   outnam, lang="e", ...)
{
	oldpar = par(no.readonly=TRUE)
	fart <- function(opar) { if (any("windows"%in%names(dev.list()))) par(opar); eop() }
	...
	if (addRawOSA) addPearOSA = FALSE  ## choose one or the other
	...
	if (useOSA) {
		...
		fappy = lapply(fbase, function(xbase) {
			...
			yappy = lapply(ybase, function(sbase) {
				...
				sobs = sbase$Obs; names(sobs) = paste0("obs",slab)    ## sums to 1
				sexp = sbase$Exp; names(sexp) = paste0("pred",slab)   ## sums to 1
				spea = sbase$Pearson; names(spea) = paste0("pear",slab); #spea = spea / sum(spea) ## doesn't make sense to standardise residuals
				## back calculate small-constant additions
				oraw = round(sobs - 0.0001, 5); oraw = oraw / sum(oraw);
				sraw = oraw - sexp;  names(sraw) = gsub("obs","raw",names(sraw))
				if (rm0) {
					## even though predictions occurred for zero values (because of small additions to AF input)
					z0 = oraw==0
					spea[z0] = sraw[z0] = 0
				}
#browser();return()
...
#browser();return()
				...
				#obs <- round(obs * yOSA[, "ESS"])  ## why round these??? doesn't make sense for proprtions-at-age
				obs <- obs * yOSA[, "ESS"]
				## extract predictions
...
#browser();return()
				if (length(sss)>1) {
					...
					mess = paste0("pear <- ",paste0(paste0("pear[,grep(\"", sss, "\",colnames(pear))]"), collapse=" + "))
					eval(parse(text=mess))
					colnames(pear) = sub(sss[1], "A", colnames(pred))
					mess = paste0("raw <- ",paste0(paste0("raw[,grep(\"", sss, "\",colnames(raw))]"), collapse=" + "))
					eval(parse(text=mess))
					colnames(raw) = sub(sss[1], "A", colnames(raw))
				}
				...
				set.seed(123)
				## calculate residuals:
				zero = obs<=0 | is.na(obs)  ## in case no constant was added in SS3 model
				if (any(zero))  obs[zero] = 0.0001
				...
				OSAlist[[ifleet]][["oas_raw"]][[iii]]  = raw   ## (RH 260226) adding raw residual for comparison with OSA
...
#if (exists("debug") && debug && !is.null(outnam)) fout.e = outnam
				if (addPearOSA) fout.e = sub("^osa", "osa+pear", fout.e)
				if (addRawOSA)     fout.e = sub("^osa", "osa+raw", fout.e)
...
#browser();return()
						...
						pearson  = if (addPearOSA) pear else NULL
						rawres   = if (addRawOSA)     raw  else NULL
						plot_cres(res, maxLag=20, oma=c(0,0,1,0), mar=c(3,3,1,1), main=lab.main, bscale=0.4, lang=l, pearson=pearson, rawres=rawres)
#browser();return()
...
#browser();return()
				...
				for ( s in 1:nsexes) {
					...
					if (useOSA) {
						sexpref = toupper(substring(names(sexes)[s],1,1))
						if (sexpref %in% c("A","B","C")) sexpref="A"
						mess = paste0("osares = OSA$fleet_", f, "$oas_res$sex_", sexpref )
#browser();return()
						eval(parse(text=mess))
   ...
   res=400, outnam, PIN=c(9,9), lang="e") 
{
	...
	if (is.null(labels)) {
		...
		labels <- c("Total biomass (t)", "Total biomass (t) by area", "Total biomass (t) at beginning of season",
			...
			"Spawning biomass (t)", "Spawning biomass (t) by area", "Relative SSB (Bt / B0)",
			"Relative SSB (Bt / B0) by area", "Age-0 recruits (1000s)", "Age-0 recruits (1000s) by area",
