...
## plotSS.compo-------------------------2022-10-11
## Make Composite Figures
## ---------------------------------------------RH
plotSS.compo = function(compo, spp.code="CAR", istock="CAR", 
...
if (sumting) {
	...
	#catpol = NULL; #avgPJ[,,"Bt",c("AC.00"),drop=F]  ## start with default catch policy (800)
	catpol = avgPJ[,,"Bt",c("CC.01","CC.04","CC.07"),drop=F]  ## CAR: see projections using catches of (0,750,1500)
...
#browser();return()
	...
	#catpol = NULL; #avgPJ[,,"BtBmsy",c("AC.00"),drop=F]  ## start with default catch policy (800)
	catpol = avgPJ[,,"BtBmsy",c("CC.01","CC.04","CC.07"),drop=F]  ## CAR: see projections using catches of (0,750,1500)
...
#browser();return()
	...
	#catpol = NULL; #avgPJ[,,"BtB0",c("AC.00"),drop=F]  ## start with default catch policy (800)
	catpol = avgPJ[,,"BtB0",c("CC.01","CC.04","CC.07"),drop=F]  ## CAR: see projections using catches of (0,750,1500)
...
so("plotSnail.r","tools")
	plotSnail(BoverBmsy, UoverUmsy, yrs=modYrs, p=tcall(quants3)[c(1,3)], xLim=NULL, yLim=NULL, ngear=length(gseries), assYrs=assYrs, outs=F, Cnames=gseries, ptypes=ptypes, outnam=outnam, lang=lang)
#browser();return()
...
## plotSS.senso-------------------------2022-09-12
## Make Sensitivity Figures
## ---------------------------------------------RH
plotSS.senso = function(senso, ptypes, spp.code="CAR", istock="CAR",
...
#browser();return()
			...
			for (l in lang) {  
				...
				for (p in ptypes) {
					...
					##par(mfrow=c(1,1), mar=c(3,3,0.5,1), oma=c(0,0,0,0), mgp=c(1.75,0.5,0))  ## mar and oma ignored, fixed in call to `mochaLatte'
					panelChains(mcmc=P.ii, axes=TRUE, pdisc=0, between=list(x=0, y=0), col.trace=c("red","blue","black"), xlab=paste0("Parameter ",ii, " value"), ylab="Cumulative Frequency", cex.axis=1.2, cex.lab=1.4, cex.strip=ccex.strip, lang=l, mar=c(2,0,0,0), oma=c(3,4,0.5,0.5)) #, yaxt="n"
					if (p %in% c("eps","png")) dev.off()
				} ## end p (ptypes) loop
			}; eop()
#browser();return()
...
#browser();return()
				xx = split(x,i);
					...
					xr[1] = quantile(x,0.01)
					xr[2] = quantile(x,0.99)
				#}
				return(xr)
			}
		}
...
#browser();return()
	...
	if (redo.figs) {
		...
		for (k in 1:Ntraj){
			...
			fout.e = paste0(prefix,"senso.traj.",kk)
			#fout.e = paste0(prefix,"senso.traj.",kk,".test.set")
...
#.flush.cat(j," ",jj," ", tlty[j], "\n")
					...
					mtext(linguaFranca(ylab,l), side=2, line=1.8, cex=ifelse(Ntraj==1,1.5,1.2))
						...
						xleg = 0.02; yleg = if (kk %in% c("U","R","RD")) 0.99 else 0.01; yjust = if (kk %in% c("U","R","RD")) 1 else 0
						addLegend(xleg, yleg, col=tcol, seg.len=5, legend=linguaFranca(legtxt,l), bty="o", box.col="grey", bg=ifelse(kk%in%c("BtB0"),"white","transparent"), xjust=0, yjust=yjust, lwd=2, lty=tlty)
					#}
					#if (Ntraj==1) {
...
#ptypes="png"; scenario=0; so("compBmsy.r")
		...
		left.space = 
			...
			else if (istock %in% c("YMR","CAR")) c(12,16)
			else c(10,10)
