...
0.4 #_spr_target -- spawner per recruit target(e.g. 0.40)
0.4 #_b_target -- spawning biomass target (e.g. 0.40)
...
#_where none and simple require no input after this line; simple sets forecast F same as end year F
...
-999 0 -999 0 -999 0 #_fcast_yrs -- years to use for averages in forecast: start and end for S; F; R (enter actual year; or values of 0 or -integer to be rel. endyr)
0 #_fcast_s_opt -- Forecast selectivity options (0=Fcast selex is mean from year range; 1=Fcast selectivity from annual time-vary parms)
1 #_ctl_rule -- method (1=ramp does catch=f(SSB); buffer on F; 2=ramp does F=f(SSB); buffer on F; 3=ramp does catch=f(SSB); buffer on catch; 4=ramp does F=f(SSB); buffer on catch)
0.002 #_ctl_rule_ul -- (upper limit) Biomass level for constant F (as frac of Bzero; e.g. 0.40); (Must be > the no F level below)
0.001 #_ctl_rule_ll -- (lower limit) Biomass level for no F (as frac of Bzero; e.g. 0.10)
0.75 #_ctl_rule_bf -- (buffer) enter Control rule target as fraction of Flimit (e.g. 0.75); negative value invokes list of [year; scalar] with filling from year to YrMax
#_COND_ctl_buff -1 -- Conditional input for annual control rule buffer (see manual)
...
2200 #_fcast_yr1 -- First year for caps and allocations (should be after years with fixed inputs) RH: set this far in future so it does not affect forecasts
...
1996 #_rebuild_catch_yr -- Rebuilder: first year catch could have been set to zero (Ydecl)(-1 to set to 1999) ignored if rebuilder=0
...
#_2 #_COND_rel_f -- Conditional input if relative F choice = 2
#_enter list of: season; fleet; relF; if used; terminate with season=-9999
#_1 1 1
#_-9999 0 0 #_end_rel_f -- terminator for list of relF
...
#_COND_alloc_grps -- if N allocation groups >0; list year; allocation fraction for each group
#_list sequentially because read values fill to end of N forecast
#_terminate with -9999 in year field
...
#_COND_==-1 -- Forecasted catches: enter one line per number of fixed forecast year catch
#_COND_>0 -- Forecasted catches: enter one line per number of fixed forecast year catch
#_enter list of Fcast catches or Fa; terminate with line having year=-9999
# RH: Based on last year of catch in 2020 of 1057 tonnnes; 5-y avg (2016-2020) = 1272 t
...
2031 1 1 1057
2032 1 1 1057
-9999 1 1 0 #_end_catpols -- year, season, fleet, catch (or_F)
