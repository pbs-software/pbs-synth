...
The base run (\Sexpr{exp.run.rwt}) for \Sexpr{name} was selected after running a range of preliminary model runs.
...
Projections out to \numberstringnum{\Sexpr{Ngen}} generations (\Sexpr{Ngen*gen1}~years), where one generation was determined to be \Sexpr{gen1}~years (see Appendix~D), were not computed because the stock status of \SPC{}, both coastwide and by area, fell unambiguously into the Healthy zone and thus did not require any rebuilding.
...
## This should really go into 'tabSS.compo.r', but routine is quick and dirty for PJS
areaRPlist = areaRPquant = list()
if (exists("xavgRP")) {
	for (a in dimnames(xavgRP)$area) {
		...
		areaRPlist[[a]]   = areaRPtab
		areaRPquant[[a]]  = apply(areaRPtab, 2, function(x){quantile(x, probs=quants5, na.rm=T)})
		write.csv(areaRPquant[[a]], paste0("subarea", a ,".csv"))
	}
#browser();return()
## lapply(areaRPquant,function(x){apply(x,2,function(y){paste0(signif(y[3],3),"(",signif(y[1],3),",",signif(y[5],3),")",collapse="")})})
}
areaQTtemp = lapply(areaRPquant,t)
areaQT     = do.call("rbind", lapply(areaQTtemp, data.frame, stringsAsFactors=FALSE))
#areaQT.out = texArray(areaQT, use.row.names=T, name.row.names="Subarea.Quant", table.caption="sumting", table.label="tab:sumting")
...
\graphicspath{{\Sexpr{paste0(appF.dir,ifelse("f" %in% lang,"/french/","/"))}}}  %% Put english figures into english/ subdirectory for CSAP runs
...
Single-area models were fit to the area-specific data from 5ABC (Queen Charlotte Sound, QCS), 3CD (west coast Vancouver Island, WCVI), and 5DE (west coast Haida Gwaii, WCHG, and Dixon Entrance), using the same assumptions as those for the multi-area model (e.g., Multinomial fit of age frequencies and one Francis mean-age reweight). 
This was also done to provide a direct link to the single-area models that were used to assess these stock-areas in the previous iterations of the BC POP stock assessment (5ABC: \citealt{Haigh-etal:2018_pop5ABC}; 3CD: \citealt{Edwards-etal:2014_pop3CD}; 5DE: \citealt{Edwards-etal:2014_pop5DE}). 
This was also done to validate and compare the area results from the multi-area model described in Section~\ref{s:POP}.

\subsubsection{5ABC -- Area 1}
%%Something from Run24v1a...
<<5ABC single-area model, echo=FALSE, eval=TRUE, results=hide>>= # hide the results 
A1.dir = paste0(model.dir, "/POP2023/Run24/")
MPD1.dir = paste0(A1.dir, "MPD.24.01.v1/")
MCMC1.dir = paste0(A1.dir, "MCMC.24.01.v1a/")
Retro1.dir = paste0(A1.dir, "Retro.24.01.v1/figs/")
@
...
The fits to the commercial AF data were similar to those obtained by the multi-area model, with few large residuals and no strong patterns in the residuals (Figure~F.38).
...
Similarly, the removal of data reveals no great surprises in the fits to the QCS synoptic index series.
This retrospective analysis did not did not materially change the fit to the QCS synoptic survey index series.
This retrospective analysis did not reveal any underlying problems in the 5ABC model, with all between-year shifts explained through the introduction of new information into the model.

\graphicspath{{\Sexpr{MPD1.dir}}}

%% #1=figure1 #2=figure2 #3=label #4=caption #5=width (fig) #6=height (fig)
\figbeside{survIndSer}{selectivity}{5abc.idx.selex}{5ABC model: survey index values (points) with 95\pc{} confidence intervals (bars) and MPD model fits (curves) for the fishery-independent survey series (left); selectivities for commercial fleet catch and surveys, with maturity ogive for females indicated by `m' (right).}{3.2}{4}
...
<<3CD single-area model, echo=FALSE, eval=TRUE, results=hide>>= # hide the results 
A2.dir = paste0(model.dir, "/POP2023/Run25/")
MPD2.dir = paste0(A2.dir, "MPD.25.01.v1/")
MCMC2.dir = paste0(A2.dir, "MCMC.25.01.v1a/")
Retro2.dir = paste0(A2.dir, "Retro.25.01.v1/figs/")
@
...
The WCVI synoptic survey captured no sub-mature fish while the NMFS triennial survey captured sub-mature fish at all ages.
(Selectivity for the WCVI historical survey was linked to that for the WCVI synoptic survey.)
The fits to the 3CD commercial AF data appeared to be better than those obtained by the multi-area model, with fewer large residuals and no strong patterns in the residuals (Figure F.43).
...
However, spawning biomass in 3CD made a greater improvement from 2010 on in the multi-area model compared with the single-area model (median $\Bcurr/B_0$~= 0.44 for the single-area model compared to 0.71 for the 3CD subarea of the multi-area model).
Strong recruitment events in 3CD occurred in 1981, 1999, 2008, and 2013 (Figure~\ref{fig:3cd.recruit.recdev}), which were the same events identified in the multi-area model for 3CD (Figure~\ref{fig:pop.compo.Rt}); however, the latter also identified 1952, which was borrowed from the data in 5ABC and 5DE.
...
There was a strong increase in biomass for 2014 and 2015, resulting from a large index value observed in 2014.
...
<<5DE single-area model, echo=FALSE, eval=TRUE, results=hide>>= # hide the results 
A3.dir = paste0(model.dir, "/POP2023/Run26/")
MPD3.dir = paste0(A3.dir, "MPD.26.01.v1/")
MCMC3.dir = paste0(A3.dir, "MCMC.26.01.v1a/")
Retro3.dir = paste0(A3.dir, "Retro.26.01.v1/figs/")
@

The fits to the WCHG synoptic survey were good, with the fit improved over the equivalent fit by the 5DE subarea in the multi-area model (Figure~\ref{fig:5de.idx.selex}).
Selectivity showed that the fishery captured sub-mature fish older than nine years (Figure~\ref{fig:3cd.idx.selex}).
The WCVI synoptic survey captured sub-mature fish older than 11 years.
The fits to the AF data (Figure F.43) are about the same as observed for the 5DE sub-model, with few outlier standardised residuals. 
...
The multi-area model (Section~xxx) indicates that the proportional split was roughly 60:20:20 among the areas, and this seemed to be reflected by the single-area models.
...
The coastwide median deviations show how the multi-area model reconciled the deviations from the three areas.
...
Thereafter, rates in 5ABC remianed at $\sim$0.05 for decades while rates reached $\sim$0.17 and $\sim$0.25 in 3CD and 5DE, respectively, during the pre-observer years in the Canadian fisheries.

Stock status of the coastwide multi-area model and the three single-area models (Figure~\ref{fig:pop.penso.stock.status}) shows some differences compared to the stock status of the multi-area subareas (Figure~\ref{fig:pop.senso.stock.status}).
The most notable is the lower status of 3CD from the single-area model, which did not have the benefit of recruitment signals from the oter two areas.
The 3CD stock still lies in the Healthy zone, but there is at least a 5\pc{} probability of lying in the Cautious zone.
At the other estreme is the higher status of the 5DE stock from the single-area model, which includes a strongly increasing abundance signal from 2016 to 2020 in the WCHG synoptic survey (Figure~\ref{fig:5de.idx.selex}).
...
<<Low recruitment scenario, echo=FALSE, eval=TRUE, results=hide>>= # hide the results 
require(r4ss)
...
Sensitivity 12 added the Hecate Strait (HS) synoptic survey index data to the third area, 5DE.
AF data from HS were minimal, with two samples of 33 aged fish in 2007.
Therefore, HS AF data were not used in the sensitivity run, and selectiviy for the HS sypotic survey was linked to the estimate for the WCHG survey.
...
Compared to S01, which was originally the base run, the Francis method yielded greater depletion (lower $\Bcurr/B_0$) and a lower $B_0/\Bmsy$, implying a lower overall productivity.
...
The implementation of the multi-area model by the SS3 platform required fixing the relative distribution of recruitment for one of the areas and then allowing the model to estimate a recruitment distributional parameter for the remaining two areas relative to the reference area. 
For the base run, the 5DE area was arbitrarily chosen as the reference area; consequently the \code{Rdist\_area(1)} parameter for the base run applied to 5ABC and the \code{Rdist\_area(2)} parameter applied to 3CD. 
Sensitivity runs S02 and S03 explored setting the reference area to 5ABC (S02) and to 3CD (S03), respectively.
In terms of the overall model performance, both S02 and S03 returned leading parameter estimates (Table F.17) and derived quantities (Table F.18) that were consistent with the base run.
As well, the fits to the survey data were similar for all three runs except for the WCVI synoptic survey, which obtained a better fit when 5ABC (S02) was the reference area (Table F.19). 
Although the overall model performance seemed to be relatively insensitive to the choice of the reference area, the relative distribution of the three subareas was sensitive to this choice.
