...
\item The assessed BC population of \Sexpr{stock.ass} comprised \Sexpr{ifelse(Nstock*Narea>1, paste0(switch(Nstock*Narea-1,"two","three","four"), " stocks in PMFC areas ", texThatVec(gsub("\\+",", ",stock.area), simplify=FALSE)), paste0(" a single stock in PMFC areas ", stock.area[1]) )}. Regional (area-specific) models were also run for comparison.
...
\comment{
%Use when two fisheries: \caption{Notation for the SS3 catch-at-age model (continued overleaf). Note: $\mr{\Sexpr{stock.let[1]}}\ms{\vee}\text{\Sexpr{stock.let[2]}}$ denotes `\Sexpr{stock.lab[1]} or \Sexpr{stock.lab[2]}'.}
}
