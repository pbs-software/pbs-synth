## -----------------------------------------------
## Controls for one or more stocks, last modified: (RH 230925)
## -----------------------------------------------
...
R.synth = c("plotFuns","utilFuns") #"PBSsynth" (migrated to zzz.r)
r.synth = c("plotSS.pars","plotSS.ts","plotSS.index","plotSS.francis", "plt.selectivity", "plotSS.selex", "make.multifig", "plotSS.comps", "plt.cohortResids", "plotSS.stdres", "plotSS.rdevs", "calcStdRes", "ptab", "convPN", "weightAF", "plotSS.pmcmc","tabDQs")
...
for (i in 1:length(stolab)) {
	...
	stock[[ii]][[jj]][["name"]]            = switch(i, paste("POP",assyr), paste("NADA",assyr)) ## Don't specify 3 stocks if using SS3's multi-area model
	stock[[ii]][[jj]][["spp.code"]]        = "POP"
	stock[[ii]][[jj]][["spp.name"]]        = "Pacific Ocean Perch"
	stock[[ii]][[jj]][["latin.name"]]      = "Sebastes alutus"
	stock[[ii]][[jj]][["area.name"]]       = switch(i, "CST", "NADA")
	stock[[ii]][[jj]][["prefix"]]          = switch(i, "pop.", "nada.") ## use istock
	stock[[ii]][[jj]][["model.dir"]]       = "C:/Users/haighr/Files/GFish/PSARC/PSARC_2020s/PSARC23/POP/Data/SS"
	stock[[ii]][[jj]][["base.dir"]]        = paste0(stock[[ii]][[jj]][["model.dir"]], switch(i, "/POP2023","/nada")) ## Top level directory for runs
	stock[[ii]][[jj]][["startYear"]]       = 1935
	stock[[ii]][[jj]][["currYear"]]        = 2024
	stock[[ii]][[jj]][["prevYear"]]        = 2023
	stock[[ii]][[jj]][["projYear"]]        = 2034  ## 10-yr projection
	stock[[ii]][[jj]][["pgenYear"]]        = 3*25  ## 3G (G=25y)
	stock[[ii]][[jj]][["gen1"]]            = 25
	stock[[ii]][[jj]][["Ngen"]]            = 3
	stock[[ii]][[jj]][["Nsex"]]            = 2
	stock[[ii]][[jj]][["Nsurv"]]           = switch(i, 6, 1)
	stock[[ii]][[jj]][["iseries"]]         = switch(i, c("QCS Synoptic", "WCVI Synoptic", "WCHG Synoptic", "GIG Historical", "NMFS Triennial", "WCVI Historical"), c("nada") )  ## index series
	stock[[ii]][[jj]][["Ncpue"]]           = switch(i, 0, 1)
	stock[[ii]][[jj]][["useries"]]         = switch(i, NA, c("nada"))  ## cpue series
	stock[[ii]][[jj]][["Ngear"]]           = switch(i, 3, 1)
	stock[[ii]][[jj]][["gseries"]]         = switch(i, c("5ABC Trawl", "3CD Trawl", "5DE Trawl"), c("nada"))  ## gear series
	stock[[ii]][[jj]][["Narea"]]           = switch(i, 3, 1)
	stock[[ii]][[jj]][["areas"]]           = switch(i, c("5ABC", "3CD", "5DE"), "BC")
	stock[[ii]][[jj]][["Nfleet"]]           = switch(i, 9, 1)
	stock[[ii]][[jj]][["fleets"]]          = switch(i, c("5ABC Trawl", "3CD Trawl", "5DE Trawl", "QCS Synoptic", "WCVI Synoptic", "WCHG Synoptic", "GIG Historical", "NMFS Triennial", "WCVI Historical"), c( "nada") )  ## fleet names
	...
	stock[[ii]][[jj]][["exp.run.num"]]     = switch(i, 21, 0) ## central run (originally called example run)
	stock[[ii]][[jj]][["exp.run.rwt"]]     = switch(i, "21.01.v3a", "")
	stock[[ii]][[jj]][["recentCatch"]]     = switch(i, list(  ## RH 230829: change to a list object for multiple areas
			'CST' =c('2018'=3845, '2019'=3754, '2020'=2973, '2021'=2360, '2022'=3599),
			'5ABC'=c('2018'=2024, '2019'=2034, '2020'=1364, '2021'=1118, '2022'=1551), 
			'3CD' =c('2018'=1066, '2019'= 711, '2020'= 970, '2021'= 606, '2022'= 849),
			'5DE' =c('2018'= 755, '2019'=1010, '2020'= 639, '2021'= 636, '2022'=1200)), 0) ## for now just provide the data
	...
	stock[[ii]][[jj]][["Nsens"]]           = switch(i, 10, 0)
	stock[[ii]][[jj]][["sseries"]]         = switch(i, rep(list(c(4:9)),10), rep(list(c(4:9)),1) )  ## survey series for comparing sens to base parameters
	stock[[ii]][[jj]][["sen.run.num"]]     = switch(i, c(17,27:35), c(0) )
	stock[[ii]][[jj]][["sen.rwt.num"]]     = switch(i, c(0,rep(1,9)), c(0) )
	stock[[ii]][[jj]][["sen.run.rwt"]]     = switch(i, c("17.00.v18a",paste0(27:35,".01.v1a")),  "")
	stock[[ii]][[jj]][["sen.lab"]]         = switch(i,
		c("D-M_parameterisation", "Rdist_5ABC_fixed", "Rdist_3CD_fixed", "AE1_no_age_error", "AE5_age_reader_CV", "AE6_CASAL_CV=0.1", "reduce_catch_30%", "increase_catch_50%", "sigmaR=0.6", "sigmaR=1.2"),
		c("lemony persnickets"))
	stock[[ii]][[jj]][["sen.long"]]         = switch(i,
		c("use Dirichlet-Mutinomial parameterisation", "fix parameter Rdist for 5ABC to 0", "fix parameter Rdist for 3CD to 0", "apply no ageing error", "use smoothed ageing error from age-reader CVs", "use constant-CV ageing error", "reduce commercial catch (1965-95) by 30\\\\pc{}", "increase commercial catch (1965-95) by 50\\\\pc{}", "reduce $\\\\sigma_R$ to 0.6", "increase $\\\\sigma_R$ to 1.2"),
		c("lemony persnickets"))
	...
	stock[[ii]][[jj]][["sen.mcsubs"]]      = switch(i, rep(list(c(1,2000)),10), rep(list(c(1,2000)),1))
