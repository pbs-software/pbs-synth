...
\graphicspath{{\Sexpr{paste0(central.mpd.dir,ifelse("f" %in% lang,"/french/","/english/"))}}}  %% Put english figures into english/ subdirectory for CSAP runs
...
retro.dir = "C:/Users/haighr/Files/GFish/PSARC24/YTR/Data/SS3/YTR2024/Run02/Retro.02.01.v2"
@
%%\graphicspath{{\Sexpr{file.path(retro.dir,ifelse("f" %in% lang,"figs/french/","figs/english/"))}}}  %% Put english figures into english/ subdirectory for CSAP runs
\graphicspath{{\Sexpr{paste0(retro.dir,ifelse("f" %in% lang,"/figs/french/","/figs/english/"))}}}  %% Put english figures into english/ subdirectory for CSAP runs
...
Projections out to \numberstringnum{\Sexpr{Ngen}} generations (\Sexpr{Ngen*gen1}~years), where one generation was determined to be \Sexpr{gen1}~years (see Appendix~D), were not computed because the stock status of \SPC{} fell unambiguously into the Healthy zone and thus did not require rebuilding.
...
Median exploitation rates reached a maximum of \Sexpr{round(ut.max.med,3)}$^{\minus \text{y}}$ in \Sexpr{names(ut.max.med)}  (Figure~\ref{fig:ytr.sbiomassMCMC}, bottom panel).
\SPC{} showed frequent recruitment events of age-0 fish (Figure~\ref{fig:ytr.recruitsMCMC}; mean of annual medians from 1935 to 2015 = \Sexpr{round(Rt.mean)}~million fish), with the largest median recruitment event in \Sexpr{names(Rt.med)[1]} of \Sexpr{round(Rt.high)}~million fish (\Sexpr{round(Rt.high/Rt.mean,1)}~times the mean of MCMC medians).
...
A phase plot of the time-evolution of spawning biomass and exploitation rate by the modelled fisheries in MSY space (Figure~\ref{fig:ytr.snail}) suggests that the stock was in the Healthy zone at the beginning of \currYear, with a current position at $B_{\currYear}/\Bmsy$ = \Sexpr{medCI(BoverBmsy[,as.character(currYear)],dig=sigdig)}
...
## PJS (241008): MPD results in MCMC section, and not referenced in the document. Remove.
...
\graphicspath{{\Sexpr{paste0(central.mcmc.dir, ifelse("f" %in% lang,"/french/","/english/"))}}}  %% Put english figures into english/ subdirectory for CSAP runs
...
A number of samples (\Sexpr{sum(is.bad)} or \Sexpr{round(100*(sum(is.bad)/N.mcmc),1)}\pc) were dropped before constructing the decision tables because the estimated MSY values were computationally undefined.
...
At current average exploitation, projected biomass will decline until it plateaus (because 4,000~t/y is close to the estimated annual median MSY of \Sexpr{formatC(round(median(avgRP[,"MSY"],na.rm=T)),big.mark=",",format="d")}\,tonnes).
...
\begin{itemize_csas}{-0.5}{}
  \item \textbf{S01}~(R\Sexpr{sen.run.rwt[1]})  -- \Sexpr{sen.long[1]}\footnote{for each sex, estimated $M$ for young fish (0-9y old) and for mature fish (10-45y old)}  %(label:~``\Sexpr{sen.lab2[1]}'');
...
<<compare_sens, echo=FALSE, eval=TRUE, results=hide>>= # hide the results 
base.curr     = senTS[grep(exp.run.rwt,rownames(senTS),invert=FALSE), as.character(currYear), ]  ## current year
base.curr.med = apply(base.curr, 2, median, na.rm=T)
base.Dmed     = base.curr.med["BtB0"]
base.rfpt.med = apply(senRP[grep(exp.run.rwt,rownames(senTS),invert=FALSE),], 2, median, na.rm=T)

## Following might be prone to error if runs are not ordered by sensitivity
sen.curr      = senTS[grep(exp.run.rwt,rownames(senTS),invert=TRUE), as.character(currYear), ]  ## current year
sen.rfpt      = senRP[grep(exp.run.rwt,rownames(senRP),invert=TRUE), ]  ## reference points
sen.samp      = sapply(strsplit(rownames(sen.curr),split="\\."), function(i){paste0(i[-length(i)],collapse=".")})
sen.idx       = match(sen.samp,sen.run.rwt)
sen.curr.med  = t(sapply(unique(sen.idx), function(i){ apply(sen.curr[grep(i,sen.idx),], 2, median,na.rm=T) }))
sen.rfpt.med  = t(sapply(unique(sen.idx), function(i){ apply(sen.rfpt[grep(i,sen.idx),], 2, median,na.rm=T) }))
rownames(sen.curr.med) = rownames(sen.rfpt.med) = sen.run.rwt
sen.run.Dmin  = which.min(sen.curr.med[,"BtB0"])
sen.run.Dmax  = which.max(sen.curr.med[,"BtB0"])
sen.Dmin      = sen.curr.med[sen.run.Dmin,"BtB0"]
sen.Dmax      = sen.curr.med[sen.run.Dmax,"BtB0"]
@
...
The median start-year (start of 2025, end of 2024) female spawning biomass relative to $B_0$ ($\Bcurr/B_0$) ranged from a low of \Sexpr{round(sen.Dmin,2)} by S\Sexpr{pad0(sen.run.Dmin,2)} (\Sexpr{gsub("\\_"," ",sen.lab[sen.run.Dmin])}) to a high of \Sexpr{round(sen.Dmax,2)} by S\Sexpr{pad0(sen.run.Dmax,2)} (\Sexpr{gsub("\\_"," ",sen.lab[sen.run.Dmax])}).
...
Other derived quantities were consistent with these observations, with S06 estimating a \Sexpr{round(100*((sen.rfpt.med[6,"B0"]/base.rfpt.med["B0"])-1))}\pc{} higher median $B_0$ than that for the base run, and the median current spawning stock size ($\Bcurr$) was \Sexpr{-round(100*((sen.rfpt.med[6,"Bcurr"]/base.rfpt.med["Bcurr"])-1))}\pc{} lower than the base run.
...
Sensitivity S07, which dropped ageing error entirely, was less optimistic in terms of percentage $B_0$ (median $\Bcurr/B_0$~= \Sexpr{round(sen.curr.med[7,"BtB0"],2)} instead of \Sexpr{round(base.curr.med["BtB0"],2)} for the base run, Table~\ref{tab:ytr.sens.rfpt}), but the overall biomass was estimated to be considerably larger in terms of absolute $B_t$ (Figure~\ref{fig:ytr.senso.traj.Bt}) than the base run (the median S07 $B_0$ was \Sexpr{round(100*((sen.rfpt.med[7,"B0"]/base.rfpt.med["B0"])-1))}\pc{} higher than base-run $B_0$, see Table~\ref{tab:ytr.sens.rfpt}). 
