...
\definecolor{colquote}{rgb}{ 0.1843137 0.3098039 0.3098039}%% darkslategrey (colour for quotes from Dr. Peoples)
...
\def\startP{169}         %% page start (default=1)
...
The 2024 stock assessment of \spn{} (\spc) adopted Stock Synthesis 3 (SS3), version 3.30.22.01 (\citealt{Methot-etal:2023}, downloaded 15 Mar 2024), which is a statistical age-structured population modelling framework \citep{Methot-Wetzel:2013} that uses ADMB's power for Bayesian estimation of population trajectories and their uncertainties.
The Stock Synthesis Development Team at NOAA (National Oceanic and Atmospheric Administration, U.S. Dept. Commerce) provides executables and documentation on how to run SS3, and the SS3 source code is available on GitHub (\code{nmfs-stock-synthesis/stock-synthesis}).

Previously, \spc{} was assessed using a simpler age-structured model called `Awatea', which is a version of Coleraine \citep{Hilborn-etal:2003} that was modified and maintained by Allan Hicks (then at Univ. Washington, now at the International Pacific Halibut Commission).
...
The running of SS3 was streamlined using custom R code (archived on the GitHub site \code{pbs-software} in the repository \code{PBSsynth}), which relied heavily on code provided by the R packages `\code{PBSawatea}', `\code{r4ss}', and `\code{adnuts}' \citep{R:2018_adnuts}.
...
\citet{Methot-Wetzel:2013} provide mathematical notation of equations used in the SS3 model in their Appendix~A.
...
\def\eec{\end{center} \\[0pt]}%% \vspace{-1ex}}
...
Input files for each model run are hosted on Github in the \code{PBSsynth} repository: \code{pbs-software/pbs-synth/}, specifically in\\
\code{./tree/master/PBSsynth/inst/input/2024/YTR/}
...
Synopsis of runs:\\
\tab \code{./00ReadMe.txt} -- Information on the runs used in 2024 YTR

Base run:\\
\tab B1~: \code{./BC/Base/B1.R02v2/02.01/} -- BC coastwide model

Sensitivity runs (MCMC):\\
\tab S01~: \code{./BC/Sens/S01.R10v2/10.01/} -- split M between ages 9 and 10 ($\sim$50\pc{} maturity)\\
\tab S02~: \code{./BC/Sens/S02.R11v2/11.01/} -- female dome-shaped selectivity\\
\tab S03~: \code{./BC/Sens/S03.R05v2/05.01/} -- reduce sigmaR to 0.6 (from 0.9)\\
\tab S04~: \code{./BC/Sens/S04.R06v2/06.01/} -- increase sigmaR to 1.2 (from 0.9)\\
\tab S05~: \code{./BC/Sens/S05.R07v3/07.01/} -- estimate sigmaR\\
\tab S06~: \code{./BC/Sens/S06.R08v2/08.00/} -- use Dirichlet-Multinomial parameterisation\\
\tab S07~: \code{./BC/Sens/S07.R12v2/12.01/} -- apply no ageing error\\
\tab S08~: \code{./BC/Sens/S08.R13v2/13.01/} -- use smoothed ageing error from age-reader CVs\\
\tab S09~: \code{./BC/Sens/S09.R14v2/14.01/} -- use constant-CV ageing error\\
\tab S10~: \code{./BC/Sens/S10.R15v2/15.01/} -- reduce commercial catch (1965-95) by 30\pc{}\\
\tab S11~: \code{./BC/Sens/S11.R16v2/16.01/} -- increase commercial catch (1965-95) by 50\pc{}\\
\tab S12~: \code{./BC/Sens/S12.R09v4/09.01/} -- use geostatistical indices for synoptic surveys\\
\tab S13~: \code{./BC/Sens/S13.R04v3/04.01/} -- add HBLL North and South surveys (longline)\\
\tab S14~: \code{./BC/Sens/S14.R17v2/17.01/} -- separate trawl fleet into bottom \& midwater trawl fleets\\

Sensitivity runs (MPD):\\
\tab S15~: \code{./BC/Sens/S15.R18v2/18.01/} -- use different priors on $M$ (with higher means)\\
\tab S16~: \code{./BC/Sens/S16.R19v1/19.01/} -- use \emph{flavidus} fecundity parameters \citep{Dick-etal:2017}\\
