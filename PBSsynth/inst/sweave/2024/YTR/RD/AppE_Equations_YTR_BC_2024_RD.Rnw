...
## --------------------------------------
base.dir   = "C:/Users/haighr/Files/GFish/PSARC24/YTR/Docs/WP/AppE_Equations" 
Nstock     = 1
Narea      = 1 ## multi-area stock assessment model (assess coastwide but use multiple areas)
Nsex       = 2
Ngear      = 1  ## even though there are 3 trawl fisheries, this is a coastwide assessment with 3 areas
spp.code   = "YTR"
spp.name   = "Yellowtail Rockfish"
stock.let  = c("YTR")
stock.lab  = c("BC")
stock.name = c("BC~coast) #c(POP~5ABC+3CD+5DE")
stock.area = c("3CD5ABCDE")
stock.subs = c("sumting")
Amax       = c(45)
#AnatM      = list(c("est"))
sigmaR     = 0.9
startYear  = 1935; startYearChar = as.character(startYear)
currYear   = 2025; currYearChar  = as.character(currYear)  ## beginning of the year
prevYear   = currYear - 1
nprojYrs   = 10  ## e.g., 10-yr projection
projYear   = currYear + nprojYrs
genTime    = 20  ## generation time (years)
glist      = list(c("BC Trawl Fishery", "QCS Synoptic", "WCVI Synoptic", "WCHG Synoptic", "HS Synoptic", "GIG Historical", "NMFS Triennial"))

## sets of model years for survey abundance indices from series g
Tlist = list(
	list(
	NA,                                                  ## No CPUE used for coastwide trawl fishery
	c(2003:2005,seq(2007,2023,2)),                       ## QCS synoptic
	c(seq(2004,2018,2),2021,2022),                       ## WCVI synoptic
	c(1997,2006:2007,seq(2008,2012,2),seq(2016,2022,2)), ## WCHG synoptic
	c(seq(2005,2023,2)),                                 ## HS synoptic
	c(seq(1967,1973,2),1976:1977,1984,1994),             ## GIG historical
	c(1980,1983,seq(1989,2001,3))                        ## NMFS triennial
	)
)
## sets of model years with proportion-at-age data for series g
Ulist = list(
	list(
	c(1979:1980,1982:1983,1985:2018),                    ## BC trawl fishery
	c(2003:2005,seq(2009,2021,2),2023),                  ## QCS synoptic
	c(seq(2006,2012,2),2016,2018,2022),                  ## WCVI synoptic
	NA, NA, NA,
	c(1983,seq(1989,2001,3))                             ## NMFS triennial
	)
)
...
avgCat       = c(4099); names(avgCat) = stock.subs ## areas adjusted for 3C expansion
...
\item The assessed BC population of \Sexpr{stock.ass} comprised \Sexpr{ifelse(Nstock*Narea>1, paste0(switch(Nstock*Narea-1,"two","three","four"), " stocks in PMFC areas ", texThatVec(gsub("\\+",", ",stock.area), simplify=FALSE)), paste0(" a single stock in PMFC areas ", stock.area[1]) )}.
...
$a$            & \mbull age class, where $a = 0, 1, 2, 3,... A$, corresponds to actual ages \Sexpr{0}, ..., \Sexpr{Amax}\\
...
\beq \elabel{Ct.year}
  C_{ats} = \sum\nolimits_{g\in\Sexpr{gcomm}} \, \frac{ F_{tg} }{ Z^{\prime}_{ats} } \, (N_{ats} S_{ags}) \lambda^{\prime}_{ats} \aeq{A.1.23}
  \eeq \\
...
%\beq \elabel{Cats}
%  C_{ats} = \sum\nolimits_{g\in\Sexpr{gcomm}}  \, \frac{ F_{tg} }{ Z_{ats}^\prime } \, w_{as} N_{ats} S_{ags} \lambda_{ats}^\prime
%  \eeq \\
...
Given the small catches in the early years (1918--1934), the model was started in \Sexpr{startYear}.
...
The bias vector used was 0.5 to \Sexpr{Amax+0.5} at increments of 1 year for ages 0 through \Sexpr{Amax}, which in SS3 signifies no age bias.
The precision vector for ages 0 through \Sexpr{Amax} was estimated as the standard deviation of ages 1 through \Sexpr{Amax+1} calculated from the CVs of lengths-at-age:
~~~$\sigma_a = a (\sigma_{L_a} / \mu_{L_a})$, where $a=0,...,\Sexpr{Amax}$.
...
However, during the current \Sexpr{spp.code} model runs, evaluated MCMC posterior samples still led to 28 out of 2000 samples (1.4\pc{}) with undefined MSY values (specified as \code{`NaN'}, or not a number).
...
This stock assessment did not use commercial CPUE because \Sexpr{spp.code} is targeted by the trawl fleet, usually using acoustic signals, leading to concerns that the series will be hyperstable.
...
Additionally, if a user needs generational forecasts (e.g, three \Sexpr{spp.code} generations = \Sexpr{genTime*3} years), then \Sexpr{genTime*3+1} forecast years need to be specified before any MCMC runs are attempted.
