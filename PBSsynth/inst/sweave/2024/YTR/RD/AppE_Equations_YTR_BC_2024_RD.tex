...
\newcommand{\LH}{}%{DRAFT (11/5/2024) not citable}%% Set to {} for final ResDoc
...
The 2024 stock assessment of \spn{} (\spc) adopted Stock Synthesis 3 (SS3), version 3.30.22.01 (\citealt{Methot-etal:2023}, downloaded 15 Mar 2024), which is a statistical age-structured population modelling framework \citep{Methot-Wetzel:2013} that uses \href{https://www.admb-project.org/}{ADMB}'s power for Bayesian estimation of population trajectories and their uncertainties.
...
Both Awatea and SS3 are platforms for implementing Automatic Differentiation Model Builder software \citep{ADMB:2009}, which provides (a)~maximum posterior density (MPD) estimates using a function minimiser and automatic differentiation, and (b)~an approximation of the posterior distribution of the parameters using the Markov Chain Monte Carlo (MCMC) method, specifically using the Metropolis algorithm \citep{Gelman-etal:2004}.
...
Additionally, this stock assessment used the safe version of SS3 (\code{`ss3\_win\_3.30.22.01.exe'}, compile date: 15 Mar 2024), which performs bound checks, rather than the `optimized' version, which is reportedly `fast and optimized for speedy execution'.
...
Figures and tables of output were automatically produced in R, an environment for statistical computing and graphics \citep{R:2024_base}. 
...
\item The assessed BC population of Yellowtail Rockfish (YTR) comprised  a single stock in PMFC areas 3CD5ABCDE.
%Independent regional (area-specific) models were also run for comparison.
\item The \spc{} fishery was dominated by trawl gear ($\sim$99.9\pc{} by catch in the last five years). Annual catches used in the model were taken by `Trawl' fisheries (bottom + midwater gear) coastwide. 
Negligible catch by other gears (halibut longline, sablefish trap, lingcod \& salmon troll, and rockfish hook \& line) was added to account for total removals. 
The annual catch was known without error and occurred in the middle of each year.
\item The Beverton-Holt stock-recruitment relationship was time-invariant, and parameterised as a beta function.
...
Fecundity-at-age for females $f_a$ was assumed to be proportional to weights-at-age $w_a$.
The default fecundity function\,: $f_a = w_a (\alpha + \beta w_a)$ (option\,1, with $\alpha$=1 and $\beta$=0) was implemented in the SS3 model for YTR.
Fecundity option\,2 should be equivalent to option\,1 ($\alpha$=1, $\beta$=0) by setting $\alpha$\,= $e^{\text{-11.347}}$ and $\beta$\,=3.071 in $f_a = \alpha L_a^{\beta}$ (using allometric parameters in Table~D.2, Appendix~D).
Sensitivity run S16 necessarily used fecundity option\,2 because \citet{Dick-etal:2017} provided length-based fecundity parameters for various \emph{Sebastes} species.
Those for \emph{S.~flavidus} were\,: $\alpha$\,= $e^{\text{-15.062}}$ and $\beta$=4.590, and the fecundity function was $f_a$\,=\,2.875e-07\,$L_a^{\text{4.590}}$.
We note that the fecundity parameters from \citet{Dick-etal:2017} for \emph{S.~flavidus} were estimated by a meta-analysis involving 456 fish taken from 8 papers spanning the years 1957 to 2013. 
All but two sampled fish were from California waters.
...
Some parameters are estimated in phase~1, then further ones in phase~2, and so on. 
...
A sensitivity run (MLE) presented at the RPR meeting (10 Sep 2024) using priors $\Norm$(0.12,\,0.036) for females and $\Norm$(0.09,\,0.027) showed a 25\pc{} increase in current spawning stock biomass ($B_{\currYear}$), a 14\pc{} increase in non-fished equilibrium biomass ($B_0$), and an increase in stock status ($B_{\currYear}/B_0$) from 0.578 to 0.628 compared to the base run MLE fit.
The priors had an effect by estimating a more optimistic stock assessment; however, the base run prior (without using the data to inform) provided a more precautionary stock assessment and was accepted by the RPR.

Normal selectivity priors were taken from those used in the 2014 YTR stock assessment, but the means were simplified (to integers) and the standard deviations were set to the mean to provide constrained uniform distributions: $\mu_{\ugees}\sim\Norm$(11,11), $\log v_{\text{L}\ugees}\sim\Norm$(2,2), and $\Delta_{\ugees}\sim\Norm$(0,10).
...
The values of $\mu_{g=4,5}$ chosen based on the distributions of female YTR length by survey (Figure D.24), which showed that females in the WCHG synoptic survey were notably larger than those from other surveys.
Initially, the WCHG age at full selectivity $\mu$ (SS3 $\beta1$) was fixed at 29.5\,y, based on a maximum length of 55\,cm and using the estimated von Bertalanffy growth function (Table D.4).
However, fixing the selectivity parameter to 29.5\,y in the population model proved to be unrealistic because the realised selectivity function lay far to the right of the maturity curve, excluding too much of the available population. 
As the prior means for $\mu_{\ugees}$ had been set to 11\,y, a choice was made to fix $\mu_{4}$\,=15\,y and to fix $\mu_{5}$\,=10\,y. 
The higher maximum age at full selectivity used for the WCHG survey recognised that this survey consistently caught larger females than those caught in other surveys (Figure D.24).
The fixed selectivity used for the HS survey was similar to but slightly smaller than the selectivity estimated for the QCS survey, given the smaller YTR observed in the HS survey length distributions (see Figures D.6 and D.24).
...
Therefore, if the end year in the data file is specified as \prevYear, derived quantities like spawning biomass $B_t$ are estimated to start of year \currYear.
...
Additionally, if a user needs generational forecasts (e.g, three YTR generations = 60 years), then 61 forecast years need to be specified before any MCMC runs are attempted.

%In this stock assessment, the default catch for projections was the average catch from 2019 to 2023, which equalled 4099\,t for sumting, respectively.
...
Base run:\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Base/B1.R02v2/02.01}{Run02v2} -- BC coastwide model

Sensitivity runs (MCMC):\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S01.R10v2/10.01}{S01.R10v2} -- split M between ages 9 and 10 ($\sim$50\pc{} maturity)\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S02.R11v2/11.01}{S02.R11v2} -- female dome-shaped selectivity\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S03.R05v2/05.01}{S03.R05v2} -- reduce sigmaR to 0.6 (from 0.9)\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S04.R06v2/06.01}{S04.R06v2} -- increase sigmaR to 1.2 (from 0.9)\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S05.R07v3/07.01}{S05.R07v3} -- estimate sigmaR\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S06.R08v2/08.00}{S06.R08v2} -- use Dirichlet-Multinomial parameterisation\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S07.R12v2/12.01}{S07.R12v2} -- apply no ageing error\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S08.R13v2/13.01}{S08.R13v2} -- use smoothed ageing error from age-reader CVs\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S09.R14v2/14.01}{S09.R14v2} -- use constant-CV ageing error\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S10.R15v2/15.01}{S10.R15v2} -- reduce commercial catch (1965-95) by 30\pc{}\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S11.R16v2/16.01}{S11.R16v2} -- increase commercial catch (1965-95) by 50\pc{}\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S12.R09v4/09.01}{S12.R09v4} -- use geostatistical indices for synoptic surveys\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S13.R04v3/04.01}{S13.R04v3} -- add HBLL North and South surveys (longline)\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S14.R17v2/17.01}{S14.R17v2} -- separate trawl fleet into bottom and midwater trawl fleets\\

Sensitivity runs (MPD):\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S15.R18v2/18.01}{S15.R18v2} -- use different priors on $M$ (with higher means)\\
\tab \href{https://github.com/pbs-software/pbs-synth/tree/master/PBSsynth/inst/input/2024/YTR/BC/Sens/S16.R19v1/19.01}{S16.R19v1} -- use \emph{flavidus} fecundity parameters from \citet{Dick-etal:2017}\\
