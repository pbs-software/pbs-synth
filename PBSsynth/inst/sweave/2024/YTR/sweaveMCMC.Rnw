...
\definecolor{greycol}{RGB}{169,169,169}%% darkgrey
...
r.synth = c("calcStdRes", "ptab", "convPN", "plotSS.pars", "plotSS.pairs", "weightAF", "plotSS.ts", "plotSS.rdevs", "plotSS.pmcmc", "calcRhat", "mochaLatte", "panelBoxes", "panelChains", "panelTraces", "plotACFs", "plotTraj")
...
\input{\Sexpr{"../../../YTR2024/runHistory"}}
...
\graphicspath{{\Sexpr{paste0(mcmc.dir,"/english/")}}}  %% Put english figures into english/ subdirectory (RH 241022)
...
if (file.exists(paste0("rhat.hist.mcmc.", run.rwt.ver, ".png"))){
	file.copy(from=paste0("rhat.hist.mcmc.", run.rwt.ver, ".png"), to="rhat.hist.mcmc.png", overwrite=TRUE, copy.date=TRUE)
	cat("\\onefig{rhat.hist.mcmc}{Stacked histogram of posteriors from eight chains for female $M$ (top left), male $M$ (top right), $\\log R_0$ (bottom left), and steepness $h$ (bottom right).}", sep="", "\n")
}
@
...
extras = list.files(".", pattern="extra.mcmc\\([[:alnum:]]+\\)-4panels.+\\.png")
if(length(extras)==0)
	extras = list.files(".", pattern="extra.mcmc\\([[:alnum:]]+\\)-3panels.+\\.png")
if(length(extras)==0)
	extras = list.files(".", pattern="extra.mcmc\\([[:alnum:]]+\\)\\(square\\)\\.png")  ## anomalous occurrence
if(length(extras)==0)
	extras = list.files(".", pattern="extra.mcmc\\([[:alnum:]]+\\)\\.png")
if(length(extras)!=0) {
	exorder = c(grep("spawn",extras), grep("deplet",extras), grep("vuln",extras), grep("exploit|harv",extras), grep("recruit",extras), grep("msy",extras))
	...
	for (i in extras[exorder]) {
		...
		if (file.exists(i)){
			ilab = paste0("Area-specific ", 
				if (grepl("spawning",i))          "spawning stock biomass (kt)"
				else if (grepl("vulnerable",i))   "vulnerable stock biomass (kt)"
				else if (grepl("depletion",i))    "depletion ($B_t/B_0$)"
				else if (grepl("harvest",i))      "harvest rate (y$^{-1}$)"
				else if (grepl("exploitation",i)) "exploitation rate (y$^{-1}$)"
				else if (grepl("recruits",i))     "recruitment (millions age-0 fish)"
				else if (grepl("frecruit",i))     "recruitment allocation"
				else if (grepl("BtBmsy",i))       "trajectories of spawning biomass ($B_t$) relative to $\\Bmsy$"
				else if (grepl("utumsy",i))       "trajectories of harvest rate ($u_t$) relative to $\\umsy$"
				)
			cat("\\onefig{", ii, "}{", ilab, ".}", sep="", "\n")
		}
	}
}
if (file.exists("extra.mcmc.snail.png")){
