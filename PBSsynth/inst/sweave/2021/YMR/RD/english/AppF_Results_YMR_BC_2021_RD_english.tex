...
\def\startP{184}         % page start (default=1)
...
\begin{itemize_csas}{}{}
  ...
  \begin{itemize_csas}{-0.25}{-0.25}
    ...
    \item B3: R75 (M=0.05)
    \item B4: R72 (M=0.055)
    \item B5: R76 (M=0.06)  %% will need testing
  \end{itemize_csas}
...
The central run of the composite base case (Run75: $M$=0.05, \cvpro=0.3296) was used as a reference case against which 14 sensitivity runs were compared.
...
Because the YMR~BC composite base case examined five models, only the central run ($M$=0.05, \cvpro=0.3296, trawl AFs adjusted by harmonic mean ratio) is presented as an example to show the fits to the data and to present MPD diagnostics (Table~\ref{tab:ymr.parest}).
...
\input{"YMR.Central.Run.MPD.relab_english"}%% Modify 'YMR.Central.Run.MPD.tex' as Sweave code relabels the references.
\clearpage
...
\input{"YMR.Central.Run.MCMC.relab_english"}%% Modify 'YMR.Central.Run.MCMC.tex' as Sweave code relabels the references.
...
\begin{landscapepage}{
\input{xtab.cruns.ll_english.txt}
\input{xtab.cruns.pars_english.txt}
}{\LH}{\RH}{\LF}{\RF} \end{landscapepage}

\begin{landscapepage}{
\input{xtab.cruns.rfpt_english.txt}
}{\LH}{\RH}{\LF}{\RF} \end{landscapepage}
...
\graphicspath{{C:/Users/haighr/Files/GFish/PSARC/PSARC_2020s/PSARC21/YMR/Docs/RD/AppF_Results/english/}}
...
\onefig{ymr.compo.rfpt.qbox}{quantile plots of selected derived quantities ($B_{\currYear}$, $B_0$, $B_{\currYear}/B_0$, MSY, $\Bmsy$, $\Bmsy/B_0$, $u_{\prevYear}$, $\umsy$, $u_\text{max}$) from 5 component runs of the base case, where each box denotes various $M$ values (0.04, 0.045, 0.05, 0.055, 0.06). The boxplots delimit the 0.05, 0.25, 0.5, 0.75, and 0.95 quantiles.}{Composite base case: }{}
...
\twofig{ymr.compo.BtB0}{ymr.compo.BtBmsy}{estimates of spawning biomass $B_t$ relative to (top) $B_0$ and (bottom) $\Bmsy$ from pooled model posteriors. The horizontal dashed lines show 0.2$B_0$ \& 0.4$B_0$ (top) and 0.4$\Bmsy$ \& 0.8$\Bmsy$ (bottom). See Fig.~\ref{fig:ymr.compo.Bt} caption for envelope details.}{Composite base case: }{}
...
\twofig{ymr.compo.ut}{ymr.compo.utumsy}{posterior distribution of (top) exploitation trajectory $u_t$ and (bottom) exploitation relative to $\umsy$.}{Composite base case: }{}

\twofig{ymr.compo.Rt}{ymr.compo.Rtdev}{posterior distribution of (top) recruitment trajectory (1000s of age-0 fish) and (bottom) recruitment deviation trajectory.}{Composite base case: }{}

\clearpage

\onefig{ymr.compo.snail}{phase plot through time of the medians of the ratios $B_t/\Bmsy$ (the spawning biomass in year $t$ relative to $\Bmsy$) and $u_{t-1} / \umsy$ (the exploitation rate in year $t-1$ relative to $\umsy$) for one fishery (trawl+). The filled green circle is the equilibtium starting year (1935). Years then proceed along lines gradually darkening from light grey, with the final year (\currYear) as a filled cyan circle, and the blue cross lines represent the 0.05 and 0.95 quantiles of the posterior distributions for the final year. Red and green vertical dashed lines indicate the PA provisional limit and upper stock reference points (0.4, 0.8 $\Bmsy$), and the horizontal grey dotted line indicates $u$ at MSY.}{Composite base case: }{}
...
While a higher $M$ may be suitable for this species, it was not supported by the available data while using the SS modelling platform.
...
\begin{landscapepage}{
\input{xtab.sens.pars_english.txt}
}{\LH}{\RH}{\LF}{\RF}\end{landscapepage}

\begin{landscapepage}{
\input{xtab.sens.rfpt_english.txt}
}{\LH}{\RH}{\LF}{\RF} \end{landscapepage}

\begin{landscapepage}{
\input{xtab.sruns.ll_english.txt}
}{\LH}{\RH}{\LF}{\RF} \end{landscapepage}
...
\onefig{ymr.senso.traj.Bt}{model trajectories of median spawning biomass (tonnes) for the central run of the composite base case and 14 sensitivity runs.}{YMR sensitivity: }{}
...
\onefig{ymr.senso.pars.qbox}{quantile plots of selected parameter estimates ($\log R_0$, $\mu_{g=1,2,3}$, $\log v_{\text{L}g=1,2}$) comparing the central run with 14 sensitivity runs. See text on sensitivity numbers. The boxplots delimit the 0.05, 0.25, 0.5, 0.75, and 0.95 quantiles; outliers are excluded.}{YMR sensitivity: }{}

\onefig{ymr.senso.rfpt.qbox}{quantile plots of selected derived quantities ($B_{\currYear}$, $B_0$, $B_{\currYear}/B_0$, MSY, $\Bmsy$, $\Bmsy/B_0$, $u_{\prevYear}$, $\umsy$, $u_\text{max}$) comparing the central run with 12 sensitivity runs (S02 and S10 omitted because biomass scale overwhelms that of the others, see Table~\ref{tab:ymr.sens.rfpt}). See text on sensitivity numbers. The boxplots delimit the 0.05, 0.25, 0.5, 0.75, and 0.95 quantiles; outliers are excluded.}{YMR sensitivity: }{}
