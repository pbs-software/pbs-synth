...
\def\startP{160}         %% page start (default=1)
...
%%----------------------------------------------------------
\\[-0.5ex]
\bec {\bf Selectivity Pattern 20 ($\bfmi{g} \bf = 1,...,5$)} \eec 
...
\beq Z_{ats} = M_{as} \sum\nolimits_{g \in 1} \, ( \, S_{atgs} \, F_{tg} \, )~;~~~ F_{tg} = \text{\footnotesize apical fishing mortality rate}
  \elabel{Zats} \eeq \\
...
%% Jon Schnute:
\beq \Phi(x|\mu,\sigma) = \frac{1}{\sqrt{2\pi}} \int_{\minus\infty}^{(x-\mu)/\sigma} e^{\minus (t^2 / 2)}\,dt~~~\text{\footnotesize cumulative normal distribution}
  \elabel{cdn.fun} \eeq \\
...
\graphicspath{{./english/}}  %% does not work if figure with same name is available on the default path (stoopid latex)
...
Even adding a CPUE series with greater contrast and somewhat lower relative error did not solve the problem of long tails associated with very large biomass estimates.
...
A wide normal prior $\Norm$(8,8) was used for $R_0$; this provided more stability in the model than using a uniform prior without affecting the estimation process.
